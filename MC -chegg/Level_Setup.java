...
public class Level_Setup {
    ...
    //     // calling the start function to setup the global variables to their default
    //     // value
    //     start();
    // }

    // // setting the global variables that are not reset every frame to their default
    // // value
    // void start() {
    //     score_reduction = 0;
    //     enemy_counter = 0;
    ...
    //     if (start == true) {
    //         // resetting the background every 3 frames to give all moving obects a sorf of
    //         // motion blur (tail)
    //         if (c == 3) {
    ...
    //         // checking each particle and handling the various flags to make the particles
    //         // move if they are alive
    ...
    //             // drawing cities and silos every frame for each particle detect collision with
    //             // each
    ...
    // void add_bomber() {
    //     bomber = new Added_enemy((float) random(width + 50, width + 250 + (200 * wave)), (float) height / 2, 40,
    ...
    //     // setting the particle's initial velocity's x coordinate as per the assigned
    //     // target

    //     switch (target) {
    //         case 0:
    //             velx = (2 * distance + distance / 192f);
    //             break;
    //         case 1:
    //             velx = (3.1 * distance + distance / 192f);
    //             break;
    //         case 2:
    //             velx = (4.2 * distance + distance / 192f);
    //             break;
    //         case 3:
    //             velx = (6.3 * distance + distance / 192f);
    //             break;
    //         case 4:
    //             velx = (7.4 * distance + distance / 192f);
    //             break;
    //         case 5:
    //             velx = (8.5 * distance + distance / 192f);
    //             break;
    //         default:
    //             velx = 0;
    //     }
    ...
    //         Particle temp[] = p;
    //         boolean tparticle_dead[] = particle_dead, tis_splitable[] = is_splitable, tis_killable[] = is_killable,
    //                 texplosion_flag[] = explosion_flag;
    //         int texplosion_counter[] = explosion_counter, texplosion_counter_temp[] = explosion_counter_temp,
    //                 texplosion_updater[] = explosion_updater, timmortality_timer[] = immortality_timer;

    //         // initializing the arrays again to store an extra particle dropped by the
    //         // bomber
    ...
    // void add_satellite() {
    //     satellite = new Added_enemy((float) random(width + 100, width + 250 + (200 * wave)), (float) height / 4, 30,
    //             (float) random(width / 2, width));
    ...
    //             velx = 0;
    //     }
    //     velx = (velx + ((distance / 4f) - (distance / 96f) - satellite.enemy.x))
    //             / (280 + (abs(satellite.enemy.y) / 30));
    //     vely = 2 + (float) wave / 32;
    ...
    // // checking the wave condition and handling what happens when the wave is passed
    // // or failed
    // void check_wave() {
    ...
    //                     if (missiles[i].desty - missiles[i].position.y <= 50
    //                             && abs(missiles[i].destx - missiles[i].position.x) <= 50
    //                             && (missiles[i].detonate() != true
    //                                     || (missiles[i].detonate() == true && detonation_counter[i] > 0))
    //                             || smart_bomb_activated == true) {

    //                         /// checking if the smart bomb is less than 50 pixels away from the incoming
    //                         /// missile
    ...
    //                         // checking if the smart bomb is approaching the incomng missile and is not too
    //                         // close to expected explosion(150 pixels)
    ...
    //                         else if ((smart_timer <= 1 && smart_bomb_activated == true)
    //                                 && (missiles[i].position.y - p[smart_bomb].position.y < 0)
    //                                 || ((tvelx > 0 && missiles[i].position.x - p[smart_bomb].position.x < -50)
    //                                         || (tvelx < 0
    //                                                 && p[smart_bomb].position.x - missiles[i].position.x < -50))) {
    ...
    //                                 p[smart_bomb].velocity = new PVector(smart_bomber_originalvelx,
    //                                         smart_bomber_originalvely);
    //                                 p[smart_bomb].invMass = 1;
    //                                 smart_bomb_activated = false;
    //                             }
    //                         } else {
    ...
    //             // if the missile reaches its destination, detonate it and make an explosion
    //             // circle
    ...
    //                 // changing the updater for the explosion radius and timer to start reducing the
    //                 // respective value to save a step and variable in updation
    ...
    //                     bomber_is_alive = bomber.check_collision(missiles[i].destx, missiles[i].desty,
    //                             detonation_counter[i]);
    //                     satellite_is_alive = satellite.check_collision(missiles[i].destx, missiles[i].desty,
    //                             detonation_counter[i]);
    ...
    //                     if (is_killable[j])
    //                         particle_dead[j] = p[j].particle_dead(missiles[i].destx, missiles[i].desty,
    //                                 detonation_counter[i]);
    //                     else if (immortality_timer[j] == 0) {
    //                         is_killable[j] = true;
    //                     } else if (immortality_timer[j] > 0) {
    //                         immortality_timer[j]--;
    //                     }
    ...
    //                     if (((particle_dead[j] == true && ptemp != true) || explosion_counter[j] != 1)
    //                             && explosion_flag[j] == false) {
    ...
    //     if (wave > 1) {
    //         boolean tempa = bomber_is_alive, tempb = satellite_is_alive;
    //         bomber_is_alive = bomber.check_collision(p[index].position.x, p[index].position.y,
    //                 explosion_counter[index]);
    //         satellite_is_alive = satellite.check_collision(p[index].position.x, p[index].position.y,
    //                 explosion_counter[index]);
    ...
    //         if (is_killable[i])
    //             particle_dead[i] = p[i].particle_dead(p[index].position.x, p[index].position.y,
    //                     explosion_counter[index]);
    //         else if (immortality_timer[i] == 0) {
    //             is_killable[i] = true;
    //         } else if (immortality_timer[i] > 0) {
    //             immortality_timer[i]--;
    //         }
    ...
    //             // calling itself to check if the explosion caused by the particle is exploding
    //             // more particles
    //             explode_particle(i);
    //             if (is_splitable[i]) {
    ...
    //     // adding particles to the existing arrays by creating temporary arrays to store
    //     // the initial values and then reinitializing the existing arrays with a new
    //     // length to include more particles
    //     Particle temp[] = p;
    //     boolean tparticle_dead[] = particle_dead, tis_splitable[] = is_splitable, tis_killable[] = is_killable,
    //             texplosion_flag[] = explosion_flag;
    //     int texplosion_counter[] = explosion_counter, texplosion_counter_temp[] = explosion_counter_temp,
    ...
    //         }
    //         fill(0);
    //         velx = (velx + ((distance / 4f) - (distance / 96f) - parent.position.x))
    //                 / (280 + (abs(parent.position.y) / 30));
    ...
    // // to set the flag for dead particles so they wont be used in any other function
    // // and wont be drawn
    ...
    //     is_killable = new boolean[p.length]; // to store if a particle is killable or not (helps while splitting
    //                                          // particles to give them time to escape the explosion they were born from)
    ...
    //         // randomly assigning a starting position to the particle above the visible
    //         // screen area
    ...
    //                 break;
    //         }
    //         cities[i] = new City(dimensions[0], dimensions[1], (dimensions[0] + dimensions[2]),
    ...
    // // checking to see if therre are any particles or cities left to decide if the
    // // wave has been won(1) or lost(2) or still going on(0)
    ...
    // // add the silos each round and reset the missile counter sso that each silo has
    // // 10 missile
    // void add_silos() {
    //     dead_missiles = 0;
    //     float dimensions[] = new float[4];
    ...
    // // missiles
    // void mouseClicked() {
    //     if (active_missile_counter + dead_missiles < 30 && check_wave_condition() != 2 && start == true
    //             && s[0].missile_counter + s[1].missile_counter + s[2].missile_counter > 0) {
    ...
    // // handling key press events to give the user control over while silo they want
    ...
    //         }
    //     }
    //     if(origin==1) {s[origin].shoot_missile((s[origin].x1+s[origin].x2)/2,s[origin].y1,(mtargetx-((s[origin].x1+s[origin].x2)/2))/23,-mtargety/23,1,mouseX,mouseY);}
    //     else {
